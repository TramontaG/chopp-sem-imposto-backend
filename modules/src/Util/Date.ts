...
export function parseDateBR(dateStr: string): Date {
  ...
  const parsedDate = new Date(
    Date.UTC(
      ...
      month - 1,
      day,
      hours + BRAZIL_UTC_OFFSET_IN_HOURS,
      minutes,
      seconds
    )
  );
  ...
  if (
    ...
    normalizedDate.getUTCHours() !== hours ||
    normalizedDate.getUTCMinutes() !== minutes ||
    normalizedDate.getUTCSeconds() !== seconds
  ) {
    return invalidDate();
  }
